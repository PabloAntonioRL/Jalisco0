...
define([
            ...
            var updateTimeFilters = function() {
                ...
                for(j=0;j<Filtros.length;j++) {
                    ...
                    layer.filter = function(feature) {
                        var f = shapeProvider.provideShape(feature);
                        var vFeatures=[];
                        for(j=0;j<Filtros.length;j++) {
                            var idLabel;
                ...
                try {
                    ...
                    for(var k =0; k<graficas.length;k++) {
                        Graficas.actualizarGraficaIdTexto(graficas[k].id, nuevaTabla, graficas[k].tipo, graficas[k].label, graficas[k].top);
                    }
    ...
    function calcMax(tabla, key) {
        ...
        var max = tabla[1][key];
        document.getElementById("nMay").innerHTML = max;
        var min = tabla[tabla.length-1][key];
        document.getElementById("nMin").innerHTML = min;
        var n = tabla.length-1;
        document.getElementById("nEventosA").innerHTML = n;
        var promedio=0;
        for(i=1; i<n; i++) {
            promedio += tabla[i][key];
        }
        promedio = Math.round(promedio/n);
        document.getElementById("nPro").innerHTML = promedio;
    }
